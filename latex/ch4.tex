...
Nous avons instanciés le workflow dans plusieurs dimensions des cas (Voir le paragraphe 1.3.4.2).         
    ...
    nous avons codé  les trois services pour l'orchestration des Micro-Service a  savoir service proxy ,service  configuration et service   registration   pour  comme le montre  la figure \ref{fig:cloudms}.
      ...
      De plus, nous avons codé un Master service paremente l'instancié le workflow pour déférent organisation c-à-d le fournisseur de service offre ses service via Master service.
 
 
     suite a la demande de la \ac{cnr} nous avons instanciés notre workflow pour cette organisation et nous avons amélioré notre application suite leur chair de charge .     
\begin{figure}[H]
	\centering
...
\end{figure}
 ...
 \subsubsection{Bpmn page }
 
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{images/captures/capturesadmin/bpmn}
	\caption{Bpmn page représenter workflow de CNR}
	\label{fig:bpmn}
\end{figure}
