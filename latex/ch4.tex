...
\end{figure}
  \subsection{Consulter les services via le proxy }
    le diagramme de séquence de la figure \ref{fig:secance}  qui exprimer la  Consultation des services via le proxy 
    \begin{enumerate}
    	
    	\item Client HTTP qui envoyer la requête HTTP vers le proxy qui contient le nom de micro service  .
    	
    	\item  Service proxy connais le nom de service , il va demander au Registration service donnez-moi la machine où se trouve ce micro service ,il va retourner l'adresse IP et le port si il a un instance si non si il a  plusieurs instance  retourner  la liste.
    	
    	\item  le service proxy va choix un instance et la va envoyer la requête  .
    	
    	\item  le service il va fer le traitement , en récupérer  le résulta  et il va envoyé ver le client .
    \end{enumerate}
